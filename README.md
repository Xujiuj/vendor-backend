...
This project is the vendor-side central backend for the carbon data management platform.

- Delivery boundary: independent Spring Boot/Maven backend under `vendor-backend`.
- Base stack: RuoYi-Vue-Plus 5.6.1, kept for RBAC, system administration, Excel utilities, and code generation.
- Current slice: backend base split only. Generated `Biz*` business CRUD from the source reference project is intentionally not included.
- Business data boundary: vendor backend manages customer records, license signing, factor versions, templates, and renewals; it must not store enterprise activity data, green electricity evidence, or intensity fact data.
