...
CREATE TABLE IF NOT EXISTS cv_factor_customer_scope (
    ...
    create_time DATETIME DEFAULT CURRENT_TIMESTAMP,
    PRIMARY KEY (id),
    UNIQUE KEY uk_cv_factor_scope_package (version_id, package_id),
    KEY idx_cv_factor_scope_lookup (version_id, scope_status, package_id),
    CONSTRAINT fk_cv_factor_scope_version
        FOREIGN KEY (version_id) REFERENCES cv_factor_version (id)
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COMMENT='Vendor factor version package scope';
...
CREATE TABLE IF NOT EXISTS cv_report_template_scope (
    ...
    create_time DATETIME DEFAULT CURRENT_TIMESTAMP,
    PRIMARY KEY (id),
    UNIQUE KEY uk_cv_report_template_scope (template_id, package_id),
    KEY idx_cv_report_template_scope_lookup (template_id, scope_status, package_id),
    CONSTRAINT fk_cv_report_scope_template
        FOREIGN KEY (template_id) REFERENCES cv_report_template (id)
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COMMENT='Vendor report template package scope';
