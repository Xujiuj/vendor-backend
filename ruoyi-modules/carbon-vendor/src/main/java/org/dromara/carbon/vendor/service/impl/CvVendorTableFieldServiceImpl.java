...
public class CvVendorTableFieldServiceImpl implements ICvVendorTableFieldService {
    ...
    private static final Set<String> ALLOWED_DIMENSION_TABLE_CODES = Set.of(
        "admin-division",
    ...
    private LambdaQueryWrapper<CvVendorTableField> buildQueryWrapper(CvVendorTableFieldBo bo) {
        ...
        lqw.in("dimension".equals(bo.getTableGroup()) && StringUtils.isBlank(bo.getTableCode()),
            CvVendorTableField::getTableCode, ALLOWED_DIMENSION_TABLE_CODES);
        lqw.in("factor".equals(bo.getTableGroup()) && StringUtils.isBlank(bo.getTableCode()),
            CvVendorTableField::getTableCode, ALLOWED_FACTOR_TABLE_CODES);
    ...
    private void validateQueryScope(CvVendorTableFieldBo bo) {
        bo.setTableGroup(trimLower(bo.getTableGroup()));
        bo.setTableCode(StringUtils.trim(bo.getTableCode()));
        if (StringUtils.isBlank(bo.getTableGroup()) || StringUtils.isBlank(bo.getTableCode())) {
            return;
        }
        if ("dimension".equals(bo.getTableGroup()) && !ALLOWED_DIMENSION_TABLE_CODES.contains(bo.getTableCode())) {
            throw new ServiceException("Unsupported vendor dimension table code: " + bo.getTableCode());
        }
        if ("factor".equals(bo.getTableGroup()) && !ALLOWED_FACTOR_TABLE_CODES.contains(bo.getTableCode())) {
            throw new ServiceException("Unsupported vendor factor table code: " + bo.getTableCode());
        }
    }
